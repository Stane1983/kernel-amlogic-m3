...
static unsigned int FN_KEY_SCANCODE = 0x300;  // 0; 
static unsigned int LEFT_KEY_SCANCODE = 0x300;  // 0;
static unsigned int RIGHT_KEY_SCANCODE = 0x300;  // 0;
static unsigned int UP_KEY_SCANCODE = 0x300;  // 0;
static unsigned int DOWN_KEY_SCANCODE = 0x300;  // 0;
static unsigned int OK_KEY_SCANCODE = 0x300;  // 0;
static unsigned int PAGEUP_KEY_SCANCODE = 0x300;  // 0;
static unsigned int PAGEDOWN_KEY_SCANCODE = 0x300;  // 0;
static unsigned int MOUSE_LEFT = 0x300;
...
{
	if((!(gp_kp->work_mode&REMOTE_WORK_MODE_FIQ))&&(!(gp_kp->work_mode&REMOTE_WORK_MODE_FIQ_SIRC)))
	{
...
static void kp_timer_sr(unsigned long data)
{
	...
	else if(kp_data->work_mode == REMOTE_WORK_MODE_FIQ_SIRC ){
		kp_send_key(kp_data->input, ~(kp_data->cur_keycode ) & 0x7f ,0);
	}
	else{
		kp_send_key(kp_data->input, (kp_data->cur_keycode>>16)&0xff ,0);
		repeat_flag = 0;
...
static  void  kp_fiq_interrupt(void)
{
	...
	else if (gp_kp->work_mode == REMOTE_WORK_MODE_FIQ_SIRC)
	{
		kp_sirc_reprot_key((unsigned long)gp_kp);
	}
	else
	{
...
work_mode_config(unsigned int cur_mode)
{
	...
	switch(cur_mode&REMOTE_WORK_MODE_MASK)
	{
		...
		case REMOTE_WORK_MODE_FIQ_SIRC:
			if((last_mode==REMOTE_WORK_MODE_FIQ)||(last_mode==REMOTE_WORK_MODE_FIQ_RCMM)||(last_mode==REMOTE_WORK_MODE_FIQ_SIRC))				break;
			...
			else if (cur_mode == REMOTE_WORK_MODE_FIQ_SIRC)
			{
				gp_kp->fiq_handle_item.handle=remote_sirc_bridge_isr;
			}
			else
			{
