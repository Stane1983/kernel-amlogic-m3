...
static void spicc_chip_select(struct amlogic_spi *amlogic_spi, bool select)
{
  ...
  if (amlogic_spi->tgl_spi->controller_data) {
    ...
    gpio_direction_output(gpio_cs, ss_pol ? select : !select);
  }
  else {
...
{	
  ...
  amlogic_spi->master_regs->conreg.data_rate_div = /*spi_clock_divideXXX*/ 3;
  printk("sys_clk=%d, spi_speed=%d, divide=%d\n", sys_clk_rate, spi_speed, spi_clock_divide);
}
