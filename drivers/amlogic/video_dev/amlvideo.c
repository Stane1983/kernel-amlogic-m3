			...
			struct v4l2_format *f)
{
    ...
    else//(fh->fmt->fourcc == V4L2_PIX_FMT_RGB32)
...
static int vidioc_dqbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	if((!vf_peek(RECEIVER_NAME)) || (b == last_pcrscr)) {
		msleep(10);
		return -EAGAIN;
	}
	last_pcrscr = b;
	...
	p->index = ppmgrvf->canvas0Addr - 0x70;
...
static int amlvideo_open(struct file *file)
{
	...
	videobuf_queue_res_init(&fh->vb_vidq, &vivi_video_qops,
					NULL, &dev->slock, fh->type, V4L2_FIELD_INTERLACED,
					sizeof(struct vivi_buffer), (void*)res);
	printk("=====kernel===open");
	return 0;
}
