...
#endif
#ifdef CONFIG_V4L_AMLOGIC_VIDEO
static void display_mode_adjust(ge2d_context_t *context, vframe_t *vf, int pic_struct)
{
    int canvas_width = ppmgr_device.canvas_width;
    int canvas_height = ppmgr_device.canvas_height;
    int vf_width = vf->width;
    int vf_height = vf->height;
    ...
    if (ppmgr_device.display_mode == 0) {//stretch full
        stretchblt_noalpha(context, 0, 0, vf_width, (pic_struct)?(vf_height/2):vf_height, 
			0, 0, canvas_width, canvas_height);
    } else if (ppmgr_device.display_mode == 1) {//keep size
        stretchblt_noalpha(context, 0, 0, vf_width, (pic_struct)?(vf_height/2):vf_height, 
			(canvas_width - vf_width)/2, (canvas_height - vf_height)/2, vf_width, vf_height);
        ...
        if (vf_width / vf_height >= canvas_width / canvas_height) {
            ...
            stretchblt_noalpha(context, 0, 0, vf_width, (pic_struct)?(vf_height/2):vf_height, 
			(canvas_width - dw) / 2, (canvas_height - dh) / 2, dw, dh);
        } else {
            dh = canvas_height;
...
#ifdef CONFIG_V4L_AMLOGIC_VIDEO
    if (ppmgr_device.receiver == 1 && mode) {
...
#ifdef CONFIG_V4L_AMLOGIC_VIDEO
        if (ppmgr_device.receiver == 0)
            stretchblt_noalpha(context,0,0,vf->width,(pic_struct)?(vf->height/2):vf->height,0,0,
