...
static ssize_t display_mode_read(struct class *cla,struct class_attribute *attr,char *buf)
{
    const char *viewmode_str[] = {"full stretch", "keep size", "4:3","16:9","1:1"};
    return snprintf(buf,80,"current display mode is %d:%s\n",ppmgr_device.display_mode,viewmode_str[ppmgr_device.display_mode]);
}

static ssize_t display_mode_write(struct class *cla,
					struct class_attribute *attr,
					const char *buf, size_t count)
{
    ssize_t size;
    ...
    if (mode >= 0 && mode < 4)
    	ppmgr_device.display_mode = mode;
    size = endp - buf;
    return count;
}
...
int  init_ppmgr_device(void)
{
    ...
    ppmgr_device.receiver_format = (GE2D_FORMAT_S32_ABGR|GE2D_LITTLE_ENDIAN);
    ppmgr_device.display_mode = 0;
#endif
    ppmgr_device.canvas_width = 640;
    ppmgr_device.canvas_height = 480;
    ppmgr_device.mirror_flag  = 0;
